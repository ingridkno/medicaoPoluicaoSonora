...
def medicao_pontos(repetibilidade_medicao, pontos_medicao, texto="tot "):
    ...
    # valores_por_ponto = []
    # horario_por_ponto = []
    lista_repetibilidade = st.columns(pontos_medicao)
    for ponto in range(pontos_medicao):
        ...
        with lista_repetibilidade[ponto]:
            ...
            for ponto_repeticao in range(repetibilidade_medicao):
                valor_medicao = st.text_input(
                    str(ponto + 1) + "." + str(ponto_repeticao + 1) + " " + texto, "0"
                )
                t_medicao = "-"
                if valor_medicao != "0":
                    t_medicao = horario()
            ...
            st.write(
                "Ponto " + str(ponto + 1) + " - média = " + str(medicao_media_db) + "dB"
            )

    return medicao_pontos_media, valores_por_ponto, horario_por_ponto


def preenche_medicoes(
    valores_por_ponto, medicao_pontos, pontos_medicao, repetibilidade, tipo_ponto_index
):
    st.write(
        valores_por_ponto,
        medicao_pontos,
    ...
    for n_pontos in range(pontos_medicao):
        ...
        # st.write(n_pontos)
        # st.write(tipo_ponto_index)
        # st.write(valores_por_ponto)
        # st.write(valores_por_ponto[inicio_rep : inicio_rep + repetibilidade])
        st.write("Medicao " + tipo_ponto_index + " - Ponto " + str(n_pontos + 1))

        relatorio.loc[
            "Medicao " + tipo_ponto_index + " - Ponto " + str(n_pontos + 1),
            colunas[3 : (3 + repetibilidade)],
        ] = valores_por_ponto[inicio_rep : inicio_rep + repetibilidade]
        relatorio.loc[
            "Medicao " + tipo_ponto_index + " - Ponto " + str(n_pontos + 1),
            colunas[3 + repetibilidade],
        ] = medicao_pontos[n_pontos]
...
def to_excel(df):
    ...
    writer = pd.ExcelWriter(
        output, encoding="utf-8", index=False, options={"remove_timezone": True}
    )
...
folium.Marker(g_lat_long, popup="Local de Medicao", tooltip=tooltip).add_to(m)
...
with st.expander("0 - Informações sobre local de medição"):
    nome_responsavel = st.text_input(
        "Nome do responsável", placeholder="Nome do responsável pela medição"
    )
    nome_estabelecimento = st.text_input(
        "Nome do estabelecimento", placeholder="Nome do estabelecimento medido"
    )
    info_local = st.text_input(
        "Informações", placeholder="Observações sobre local de medição"
    )
...
with st.expander("1 - Atendimento dos requisitos ambientais"):
    ...
    instrumento = st.checkbox(
        "Umidade relativa do ar, velocidade do vento e temperatura de acordo com especificações dos instrumentos de medição",
        value=True,
    )

    if False in ([chuva_trovoada, veloc_vento, temperatura, instrumento]):
        justificativa_ambiental = st.text_input(
            "Justificativa",
            placeholder="Justifique sobre necessidade de fazer medição em condições adversas",
        )
        t_01 = horario()
    else:
        justificativa_ambiental = (
            "Requisitos ambientais não foram intereferentes na medição"
        )

with st.expander("2 - Definição do tempo de medição e integração"):
    st.write(
        "O tempo de medição deve permitir a caracterização sonora do objeto de medição, abrangendo as variações sonoras durante seu funcionamento ou ciclo."
    )
...
with st.expander("3 - Definição de pontos de medição"):
    pontos_medicao_local = st.number_input(
        "Número de pontos de medição do emissor", value=3, step=1
    )
    st.write("Nº de pontos de medição do emissor = ", pontos_medicao_local)

    pontos_medicao_residual = st.number_input(
        "Número de pontos de medição do ruído residual", value=1, step=1
    )
    ...
    option_tipomedicao = st.radio("Tipo de medição", tipos_medicao)
    ...
    if option_tipomedicao == tipos_medicao[2]:
        ...
        justificativa_tpmedicao = (
            checagem_mobilia.replace("com", com_ou_sem) + ", k= " + str(k)
        )
...
with st.expander("4 - Definição do método de medição"):
    ...
    option_metodomedicao = st.radio("Método de medição", metodos)
...
with st.expander("5 - Ajuste do sonômetro"):
    st.write(
        "Ajuste o sonômetro com o calibrador sonoro acoplado ao microfone, imediatamente antes de cada série de medições com o valor indicado no certificado de calibração."
    )
    st.write(
        "O ajuste deve ser realizado no local das medições e isento de interferências sonoras que possam influenciá-lo."
    )
    st.write(
        "Ao final de cada série de medições, deve ser lido o nível de pressão sonora com o calibrador sonoro ligado e acoplado ao microfone."
    )
    st.write(
        "**Se a diferença for maior ou inferior ao valor absoluto de 0,5 dB, os resultados devem ser descartados e novas medições realizadas.**"
    )

    mensagem_ajuste_true = (
        "Ajuste do sonômetro realizado e dentro da diferença de 0,5 dB"
    )
    ajuste_realizado = st.checkbox(mensagem_ajuste_true, value=True, key="aj1")
...
if ajuste_realizado:
    with st.expander("6 - Definição do nível de pressão sonora"):
        ...
        if option_metodomedicao == metodos[0]:
            ...
            (
                medicao_pontos_tot,
                valores_por_ponto_tot,
                horario_por_ponto_tot,
            ) = medicao_pontos(repetibilidade_medicao, pontos_medicao_local)
            ...
            (
                medicao_pontos_res,
                valores_por_ponto_res,
                horario_por_ponto_res,
            ) = medicao_pontos(
                repetibilidade_medicao, pontos_medicao_residual, texto="res "
            )
            ...
            if (media_NPS_total - media_NPS_residual) > 15:
                ...
                subtracao_energia_db = calculo_subtracao_energia_db(
                    media_NPS_total, media_NPS_residual
                )
                Lesp = subtracao_energia_db
                mensagem_calculo_especifico = (
                    "3 dB <= NPS Total - NPS Residual < 15 dB "
                )
...
with st.expander("7 - Ajuste do sonômetro"):
    st.write(
        "Ajuste o sonômetro com o calibrador sonoro acoplado ao microfone, imediatamente antes de cada série de medições com o valor indicado no certificado de calibração."
    )
    st.write(
    ...
    ajuste_realizado = st.checkbox(mensagem_ajuste_true, value=True, key="aj2")


with st.expander("8 - Avaliação sonora em ambientes externos"):
    ...
    if LR_medido_emissor > NPS_aceitavel:
        msg_nps_comparacao = (
            "**NPS medido** ("
            + str(LR_medido_emissor)
            + " dB) > "
            + str(NPS_aceitavel)
            + " dB"
        )
        msg_dentro_norma = (
            "Medição **acima do nível aceitado** para "
            + periodo_procurado.lower()
            + " em "
            + tipo_area_medida.lower()
            + "."
        )
        ...
        msg_dentro_norma = (
            "Medição **dentro do nível aceitado** para "
            + periodo_procurado.lower()
            + " em "
            + tipo_area_medida.lower()
            + "."
...
# colunas = ["Relatorio", "Justificativa", "Horario"] + (
#     colunas_medicao + ["NPS resultante"] + horario_medicao
# )
...
# relatorio.loc["Tipo medicao", colunas[0:2]] = [
#     option_tipomedicao,
#     justificativa_tpmedicao,
# ]
...
#     valores_por_ponto_tot,
#     medicao_pontos_tot,
#     pontos_medicao_local,
#     repetibilidade_medicao,
#     "Emissor",
# )
# preenche_medicoes(
#     valores_por_ponto_res,
#     medicao_pontos_res,
#     pontos_medicao_residual,
#     repetibilidade_medicao,
#     "Ruido Residual",
# )
